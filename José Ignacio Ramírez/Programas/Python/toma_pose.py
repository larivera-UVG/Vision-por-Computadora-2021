...
12/08/2020: Version 0.3.0 -- Se agregan en la funcion getRobot_fromSnapshot() se agrega un modo para debugging.
                             Este modo permite al usuario debugear y mostrar todas las imagenes (no recomendado
                             en multi-hilos), mostrar solo las imagenes finales (puede usarse con multi-hilos)
                             o no mostrar nada (el modo capture). Se arregla tambien la ubicacion del robot en la
...
def process_image(calib_snapshot, Canny_inf, Canny_sup):
    ...
    contour, hierarchy = cv.findContours(canny_img, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    ...
    for c in contour:
        ...
        if ctrl == 1:
            ...
            if a == 0:
                ...
                """
                Una vez encuentra las esquinas, se ubica en medio de cada cuadro para tener un poco mas de exactitud en
                el numero que va a tomar como referencia. Por lo tanto, ubica el centro del cuadoro identificado y
                busca el numero que corresponde a ese pixel (dependiendo la iluminacion el gris puede variar entre
                75 - 90 o 100 a 150)
                """
            ...
            """
            Luego, se ubica cada cuadro manualmente, es decir, desde a0 hasta a7, se busca dentro de la imagen. Esto se hace manual para tener un
            mejor control de la ubicacion y que sea mas facil cambiarlo si se desea.
            Al igual que con las esquinas, se toma el valor medio del cuadro (tratando de buscar justo el centro) para tener un valor
            de referencia de que color se esta identificando (en este caso, negro o gris para 0 o 1 respectivamente).
            """
            ...
            """
            La extraccion del codigo se hace mas facil utilizando las funciones dentro de python para el manejo de numeros binarios
            y decimales. Se define un TRESHOLD_DETECT_MAX y TRESHOLD_DETECT_MIN para que el valor que tome como gris se adecuado.
            Estos valores pueden variar segun la luz (normalmente varia TRESHOLD_DETECT_MIN)
            """
